...
global {
	...
	// Create new road
	action create_toy_fake_road (int road_start_node_index, int road_end_node_index) {
		create Road {
			// Get start crossroad
			start_node <- Crossroad[road_start_node_index];
			ask start_node {
			...
			// Create road shape
			shape <- line(start_node.location, start_node.location);		
			road_type <- "fake";
		}
...
species Road skills: [scheduling] {	
	...
	// Type
	string road_type <- "normal" among: ["normal", "fake"];
	...
	action add_waiting_agents (date request_time) {
		...
		loop while: not empty(waiting_cars) and is_joinable() {
			// Get first car			
	...
	// True if accessible
	bool is_joinable {
		return has_capacity() and start_node.accessible; 
	}
	...
	action compute_travel_time(Car car) {
		ask car {
			max_speed <- get_max_freeflow_speed(myself);
			free_flow_travel_time <- myself.length / max_speed;
			travel_time <- free_flow_travel_time * (road_alpha + road_beta * (myself.jam_percentage ^ road_gamma));			
		}
	}
